...
<body>
    <div class="invoice-wrapper">
        <div class="container mt-3">
            ...
            <div class="row">
                ...
                <div class="col-6 text-end">
                    <div><b>{{ __('sale.order.order') }}: #{{ $sale->order_code  }}</b></div>
                    <div><b>{{ __('app.date') }}: {{ $sale->getFormattedSaleDateAttribute()  }}</b></div>
            ...
            <table class="table table-sm mt-2">
                ...
                <tbody>
                    ...
                    <!-- <tr class="text-end fw-bold">
                        <td></td>
                        <td colspan="2">{{ __('app.total_quantity') }}: {{ $formatNumber->formatQuantity($totalQty) }}</td>
                        <td colspan="2">{{ __('app.total_amount') }}: {{ $formatNumber->formatQuantity($sale->grand_total) }}</td>
                        <td>{{ $formatNumber->formatQuantity($totalQty) }}</td>
                        <td>{{ $formatNumber->formatWithPrecision($sale->grand_total) }}</td>
                    </tr>--> 
                </tbody>
     ...
     <div class="container mt-3 mb-3 hide-print-btn">
        <a class="btn btn-success print-btn" href="{{ url('sale/order/list') }}" onclick="window.close();">Back</a>
    </div>
