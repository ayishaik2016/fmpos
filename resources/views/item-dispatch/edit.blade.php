...
@section('title', __('warehouse.item_dispatch'))
        ...
        <div class="page-wrapper">
            <div class="page-content">
                ...
                <div class="row">
                    <form class="g-3 needs-validation" id="itemDispatchForm" action="{{ route('item_dispatch.update') }}" enctype="multipart/form-data">
                        {{-- CSRF Protection --}}
                        @csrf
                        @method('PUT')

                        <input type="hidden" name="item_dispatch_id" value="{{ $itemDispatch->id }}">
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body p-4 row g-3">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <!--  -->
                                            <div class="input-group mb-3">
                                                <x-input type="text" name="prefix_code" :required="true" placeholder="Prefix Code" value="{{ $itemDispatch->prefix_code }}"/>
                                                <span class="input-group-text">#</span>
                                                <x-input type="text" name="count_id" :required="true" placeholder="Serial Number" value="{{ $itemDispatch->count_id }}"/>
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <x-label for="vehicle_id" name="{{ __('vehicle.vehicle') }}" />
                                            <x-dropdown-vehicle selected="{{ $itemDispatch->vehicle_id }}" dropdownName="vehicle_id" />
                                        </div>
                                        <div class="col-md-4">
                                            <x-label for="driver_id" name="{{ __('vehicle.driver') }}" />
                                            <x-dropdown-roleuser selected="{{ $itemDispatch->driver_id }}" dropdownName="driver_id" roleName="{{ $driverId }}" :showOnlyUsername='false' />
                                        </div>
                                        <div class="col-md-4">
                                            <x-label for="salesman_id" name="{{ __('vehicle.salesman') }}" />
                                            <x-dropdown-roleuser selected="{{ $itemDispatch->salesman_id }}" dropdownName="salesman_id" roleName="{{ $salesmanId }}" :showOnlyUsername='false' />
                                        </div>
                                        <div class="col-md-4">
                                            <x-label for="reference_no" name="{{ __('app.reference_no') }}" />
                                            <x-input type="text" name="reference_no" :required="false" placeholder="(Optional)" value="{{ $itemDispatch->reference_no }}"/>
                                        </div>
                                    </div>
                                    <div class="card-header px-4 py-3">
