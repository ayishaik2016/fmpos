"use strict";
    ...
    $('#vehicle_id').on('change', function() {
        var vehicleId = $('option:selected', this).val();

        $.ajax({
            url: baseURL + '/item-dispatch/transaction/' + vehicleId,
            method: 'GET',
            data: {},
            beforeSend: function() {
                showSpinner();
    ...
    function addRowToInvoiceItemsTable(recordObject, loadedFromUpdateOperation=false){
       //Find service table row count
        ...
        var stockInUnit   = '<input type="text" disabled name="stockInUnit['+ currentRowId +']" class="form-control" value="' + _parseQuantity(recordObject.stock_in_unit) + '">';
