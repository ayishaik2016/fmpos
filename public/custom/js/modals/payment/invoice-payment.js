$(function() {
    ...
    */
    ...
    function handlePaymentResponse(response) {
        ...
        //Payment Type
        makePaymentForm.find('#payment_type_id option:selected').remove();
        
        //Payment Attachment
        makePaymentForm.find('input[name="payment_attachment"]').val('');

        //Attachment Preview
        makePaymentForm.find('.imagePreview, .pdfPreview').hide().attr('src', '');
    ...
    function handleHistoryResponse(response, showModel = true) {
        ...
        var invoiceSummary = `<a href="${baseURL}/payment/sale-invoice/summary/${response.data.invoice_id}" target="_blank" class="btn btn-primary">Invoice Summary</a>
                            <a href="${baseURL}/payment/sale-invoice/detail/${response.data.invoice_id}" target="_blank" class="btn btn-primary mt-1">Invoice Payment Detail</a>`;
                            
        $('.invoice_payment_history').html(invoiceSummary);
        ...
        $.each(response.data.paymentTransactions, function(index, payment) {
            ...
            if(payment.attachment) {
                attachment = `<a download href="${baseURL}/payment/sale-invoice/getimage/${payment.attachment}" target="_blank" class="text-primary" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Download"><i class="bx bxs-download"></i></a>`;
            }
            var newRow = `
