"use strict";
    ...
    function getSaleReturnAmount(){
        return $(".return_sale_amount").val();
    }
    ...
    function setBottomInvoiceTotal(){
        ...
        if ($('#return_sale').prop("checked") == true) {
            var saleReturnAmount = parseFloat(getSaleReturnAmount());
            if(saleReturnAmount >= sumOfTotalColumn) {
                saleReturnAmount = sumOfTotalColumn;
                $(".return_sale_amount").val(saleReturnAmount);
            }
            sumOfTotalColumn -= parseFloat(saleReturnAmount);
        }
    ...
    $(document).on('change', '.return_sale_amount', function(){
        setBottomInvoiceTotal();
    });
    ...
    * */
    ...
    $('.payment_type_1').on('change', function() {
        var selectedValue = $('option:selected', this).val();
        $('.payment_attachment_div').addClass('d-none');

        if(selectedValue >= 2) {
            $('.payment_attachment_div').removeClass('d-none');
        }
    });
    ...
    */
    ...
    function loadAttachmentBrowser(uploadedAttachment, accountFileInput, accountAttachmentReset) {
        if (uploadedAttachment.length) {
            accountFileInput.on("change", function() {
              if (accountFileInput[0].files[0]) {
                const file = accountFileInput[0].files[0];
                const fileUrl = URL.createObjectURL(file); 
                const fileType = file.type;

                uploadedAttachment.find('.imagePreview, .pdfPreview').hide().attr('src', '');
                if (fileType.startsWith('image/')) {
                    uploadedAttachment.find('.imagePreview').attr('src', fileUrl).show();
                } else if (fileType === 'application/pdf') {
                    uploadedAttachment.find('.pdfPreview').attr('src', fileUrl).show();
                }
              }
            });

            accountAttachmentReset.on("click", function() {
    ...
    $(document).on('change', '#vehicle_id', function() {
        var selectedData = $('#vehicle_id').select2('data')[0]; 
        if (!selectedData) {
            return true;
        }
    ...
    $(document).on('click', '#return_sale', function() {
        $('.return_sale_amount').val(0);
         $('.return_sale_amount').prop('disabled', true);
         $('.return_sale_div').addClass('d-none');
        if ($(this).prop("checked") == true) {
            $('.return_sale_amount').prop('disabled', false);
            $('.return_sale_div').removeClass('d-none');
        } 
    });

    $(document).ready(function() {
        loadAttachmentBrowser($("#uploaded-attachment-1"), $(".input-box-class-1"), $(".attachment-reset-class-1"));
    });
