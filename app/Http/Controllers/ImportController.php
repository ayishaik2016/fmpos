...
class ImportController extends Controller
{
    ...
    public function itemUpdation()
    {
        $path = public_path('ItemList.xlsx');
        ...
        try{
            ...
            for($i=0; $i < $totalCount; $i++) {
                ...
                $itemCode = $data[$i][0];
                $itemName = $data[$i][1];
                $purchasePrice = $data[$i][2];
                $salePrice = $data[$i][3];

                if(isset($itemDetail[$itemCode])) {
                    $itemData = $itemDetail[$itemCode];
            ...
            echo "Item Updation Successfully";
    ...
    public function itemCustomerPrice()
    {
        $path = public_path('ItemList.xlsx');
        $spreadsheet = $this->reader->load($path);
        $sheetNumberOne = 0;
        ...
        try{
            ...
            $customerArr = array_filter($data[0], fn($value) => !is_null($value) && $value !== '');
            $customers = Party::where('party_type', 'customer')->whereIn('first_name', $customerArr)->get();
            foreach($customers as $customer) {
                $partyDetail[$customer->first_name] = $customer->id;
            }

            echo "<pre>";
            // print_r($customerArr);
            print_r($partyDetail);
            echo count($partyDetail);
            die;
            $customerArr = array_slice($customerArr, 61, 90);
            foreach($customerArr as $key => $value) {
                ...
                for($i=0; $i < $totalCount; $i++) {
                    ...
                    $itemCode = $data[$i][0];
                    $fetchData = $data[$i][$key];

                    if(isset($itemDetail[$itemCode])) {
                        $customerData = $partyDetail[$value];
                        $itemData = $itemDetail[$itemCode];
                        

                        if(isset($customerData) && isset($itemData)) {
                            if($itemData['sale_price'] != $fetchData) {
                                CustomerItem::create([
                                    'item_id' => $itemData['item_id'],
                                    'party_id' => $customerData,
                                    'item_code' => $itemData['item_code'],
                                    'name' => $itemData['name'],
                                    'sale_price' => $itemData['sale_price'],
                                    'purchase_price' => $itemData['purchase_price'],
                                    'customer_item_price' => $fetchData,
                                    'status' => 1,
                                    'created_at' => date('Y-m-d H:i:s')
                                ]);

                                // $customerItemPrice[] = array(
                                //     'item_id' => $itemData['item_id'],
                                //     'party_id' => $customerData,
                                //     'item_code' => $itemData['item_code'],
                                //     'name' => $itemData['name'],
            ...
            // echo "<pre>";print_r($customerItemPrice);die;

            echo "Customer Item Creation Successfully";
