...
class PartyController extends Controller
{
    ...
    public function store(PartyRequest $request)  {
        try {
            ...
            if ($request->hasFile('avatar') && $request->file('avatar')->isValid()) {
                $recordsToSave['avatar'] = $this->uploadImage($request->file('avatar'));
            }
    ...
    private function uploadImage($image) : String{
        ...
        Storage::putFileAs('public/images/party/avatar', $image, $filename);
    ...
    public function getAjaxSearchBarList(Request $request)
    {
        ...
        $parties = $query
            ->select('id', 'first_name', 'last_name', 'mobile', 'is_wholesale_customer', 'currency_id', 'avatar')
            ->offset($offset)
            ->limit($perPage)
            ->get();
        ...
        $results = $parties->map(function ($party) {
            ...
            return [
                ...
                'mobile' => $party->mobile,
                'avatar' => url('/party/getimage/' . $party->avatar),
