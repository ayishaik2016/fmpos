...
class SaleTransactionReportController extends Controller
{
    ...
    function getSalePaymentRecords(Request $request): JsonResponse{
        try{
            ...
            $preparedData = Sale::with([
                'party',
                'paymentTransaction' => function ($query) use ($fromDate, $toDate, $paymentTypeId, $userId) {
            ...
            ->when($fromDate, function ($query) use ($fromDate, $toDate) {
                $query->whereHas('paymentTransaction', function ($q) use ($fromDate, $toDate) {
            ...
            ->when($invoiceNumber, function ($query) use ($invoiceNumber) {
                $query->where('sale_code', $invoiceNumber);
            })
            ->get();
            // $preparedData = Sale::with('party', 'paymentTransaction') ->when($fromDate, function ($query) use ($fromDate, $toDate) { return $query->whereHas('paymentTransaction', function ($query) use ($fromDate, $toDate) { $query->whereBetween('transaction_date', [$fromDate, $toDate]); }); }) ->when($partyId, function ($query) use ($partyId) { return $query->where('party_id', $partyId); }) ->when($paymentTypeId, function ($query) use ($paymentTypeId) { return $query->whereHas('paymentTransaction', function ($query) use ($paymentTypeId) { return $query->where('payment_type_id', $paymentTypeId); }); }) ->get();
            ...
            foreach ($preparedData as $data) {
                foreach($data->paymentTransaction as $transaction){
                    $recordsArray[] = [
                                    'payment_date'          => $this->toUserDateFormat($transaction->transaction_date),
                                    'attachment'            => $transaction->payment_attachment,
                                    'reference_no'          => $transaction->reference_no ?? '',
                                    'transaction_date'      => $this->toUserDateFormat($data->sale_date),
                                    'invoice_or_bill_code'  => $data->sale_code,
                                    'invoice_id'            => $data->id,
                                    'party_name'            => $data->party->getFullName(),
