...
class SaleController extends Controller
{
            ...
            if($request->operation == 'save' || $request->operation == 'convert'){
                ...
                if($itemDispatchId) {
                    ...
                    if(!empty($itemDispatchDetail)){
                        $validatedData['item_dispatch_id'] = $itemDispatchId;
                    }
                }
                ...
                $fillableColumns = [
                    ...
                    'return_sale'           => $validatedData['return_sale'] ?? 0,
                    // 'return_sale_amount'    => $validatedData['return_sale_amount'],
                    // 'sales_return_id'       => $validatedData['sales_return_id'],
                ];

                if(isset($validatedData['return_sale']) && $validatedData['return_sale'] == 1) {
                    $fillableColumns['return_sale_amount'] = $validatedData['return_sale_amount'];
                    $fillableColumns['sales_return_id'] = $validatedData['sales_return_id'];
                }
                ...
                if($itemDispatchId) {
                    $itemDispatchDetail = ItemDispatch::where('id', $itemDispatchId)->first();
                    if(!empty($itemDispatchDetail)){
                        $fillableColumns['item_dispatch_id'] = $itemDispatchId;
                    }
                }
    ...
    public function saveSalePayments($request)
    {
        ...
        for ($i=0; $i <= $paymentCount; $i++) {
            ...
            if($amount > 0){
                ...
                $isAttachment = 0;
                if($request->payment_type_id[$i] >= 2) {
                    $isAttachment = 1;
    ...
    public function saveSaleItems($request)
    {
        ...
        if($itemDispatchId) {
            $itemDispatchDetail->total_sale_price += $request->grand_total;
    ...
    public function datatableList(Request $request){
        ...
        return DataTables::of($data)
                    ...
                    ->addColumn('action', function($row){
                            ...
                            if($row->return_sale == 1) {
                                $salesReturnUrl = route('sale.return.details', ['id' => $row->sales_return_id]);
                                $salesReturn = '<li>
                                    <a class="dropdown-item" href="' . $salesReturnUrl . '"></i><i class="bx bx-show-alt"></i> '.__('sale.sale_return').'</a>
                                </li>';
                            }
